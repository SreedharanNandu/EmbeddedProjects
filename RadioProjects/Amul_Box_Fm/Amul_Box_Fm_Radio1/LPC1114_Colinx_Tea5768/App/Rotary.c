...
#define RADIO_ENC_EDGE_DELAY         ((20) )
#define RADIO_ENC_EDGE_FASTER        (8u)   ///* 2 seconds / 8 = 0.25 second*/
#define RADIO_ENC_EDGE_DELAY_EXPIRED (0u)

///* Use only encoder Gray code phase A to count the detents*/
#define RADIO_ENC_USE_ENCODER_B_TO_COUNT (0)
...
{  
   ...
   Signal_A = GPIOGetValue(PORT0,8);
   Signal_B = GPIOGetValue(PORT0,9);
...
{
   Sample_Encoder();
   if(Get_Timer_Status(TMR6) == TMR_EXPIRED)
   {
      Encoder_Periodic();
...
void Decode_Encoder( void )
{
   ...
   if ( Encoder_State == RADIO_ENC_MINIMUM_ENCODER_STATE )
   {
      ...
      Encoder_Count++ ;
      //RIGHT_RADIO_TURN;
      //UART_Printf("\n\r Positive Rot Counts:%d ", Encoder_Count);
      Execute_Commands('+');   
   ...
   else if ( Encoder_State == RADIO_ENC_MAXIMUM_ENCODER_STATE )
   {
      ...
      Encoder_Count--;
      //LEFT_RADIO_TURN;
      //UART_Printf("\n\r Negative Rot Counts:%d ", Encoder_Count);
      Execute_Commands('-');   
