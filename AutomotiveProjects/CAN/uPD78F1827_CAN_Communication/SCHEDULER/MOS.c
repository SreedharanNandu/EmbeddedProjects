...
*/  
 ...
 static u16 fl_fast_rate_tt_counters_U16A[ MOS_NUM_FAST_RATE_TASKS ]; 
 #if (MOS_TT_FAST_TICK_RATE > 1)
 static u16 fl_fast_rate_tt_tick_counter_U16;                         
#endif                                                                   
...
static const u8 fl_trigger_mask_U8A[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
#endif
...
static u16 fl_slow_rate_tt_counters_U16A[ MOS_NUM_SLOW_RATE_TASKS ];
static u16 fl_slow_rate_tt_tick_counter_U16;
#endif

#if defined(MOS_TIMED_TASKS_CONFIGURED)
static u16 fl_timed_task_trigger_bits_U16;
static u16 fl_copy_timed_task_trigger_bits_U16;
#endif

#if MAX_SW_TIMER_ID > 0
volatile static u16 fl_time_stamp_U16;
volatile static u16 fl_timer_base_counter_U16;
static u16 fl_sw_timers_U16A[ MAX_SW_TIMER_ID ]; 
static u8 fl_sw_timer_status_U8A[ MAX_SW_TIMER_ID ]; 
#endif
...
**********************************************************************************/
#ifdef _COSMIC_COMPILER
/* 
** The below snippet of code is compiler specific. 
** Intention is to place variables gl_stack_lower_bound_U8
** and gl_stack_upper_bound_U8 and the addresses specified by Macros
** MOS_STACK_LOWER_BOUND and MOS_STACK_UPPER_BOUND respectively.
*/
vu8 gl_stack_lower_bound_VU8 @(MOS_STACK_LOWER_BOUND);

vu8 gl_stack_upper_bound_VU8 @(MOS_STACK_UPPER_BOUND);

vu8 gl_RAM_Check_Value_VU8 @(MOS_RAM_START_ADDRESS);

vu32 gl_RAM_Address_VU32 @(MOS_RAM_START_ADDRESS+1);
#endif
#ifdef _IAR
...
** MOS_STACK_LOWER_BOUND and MOS_STACK_UPPER_BOUND respectively.
*/
__no_init vu8 gl_stack_lower_bound_VU8 @(MOS_STACK_LOWER_BOUND);

__no_init vu8 gl_stack_upper_bound_VU8 @(MOS_STACK_UPPER_BOUND);

__no_init vu8 gl_RAM_Check_Value_VU8 @(MOS_RAM_START_ADDRESS);

__no_init vu32 gl_RAM_Address_VU32 @(MOS_RAM_START_ADDRESS+2);
...
void main(void)
{
  ...
  /* Disable all Interrupts */  
   ...
   /*  
   ** Initialize stack boundries
   */
   gl_stack_lower_bound_VU8 =  MOS_STACK_BOUNDRY_VAL;
   gl_stack_upper_bound_VU8 =  MOS_STACK_BOUNDRY_VAL;
   /*
