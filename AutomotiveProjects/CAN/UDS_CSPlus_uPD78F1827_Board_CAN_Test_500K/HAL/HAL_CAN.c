...
const u32 SupportedRxCANID[1]= {
                                 SET_STD_ID(CAN_RX_ID),
                               };
/******************************************************************************
...
void CanReceive(void)
{
   ...
   if((M[rxObjHandle].CTRL & kMCTRL_DN)==kMCTRL_DN)
   {
      for(i=0;i<8;i++)
      {
         CanHALRxobject.data[i] = M[rxObjHandle].DATA[i];
      }
      CanHALRxobject.DLC =  M[rxObjHandle].DLC;

      CanReadCANId((u8)rxObjHandle, &CanRxActualId);
      ...
      CANTP_ProcessRxFrame(CanHALRxobject.data,CanHALRxobject.DLC);
      
      /* Release receive message object */
      CanWriteMCTRL((u8)rxObjHandle, (u16)kClearMCTRL_DN);
   }
